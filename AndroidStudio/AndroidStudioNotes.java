...
- #implicit_intents
- #room  (data persistence using ORM)
...
1. Add the dependcies (cannot for the life of me find the dependencies string)
    In the app-level build.gradle file:
    // Add Room dependencies
    implementation "android.arch.persistence.room:runtime:1.1.1"
    annotationProcessor "android.arch.persistence.room:compiler:1.1.1"

2. Create a class which will act as a table with its attributes:
                                    //adding @Entity initializes this class as a table.
    @Entity(tableName = "task")     //If tableName isn't specified the classname is used for the table's name
    public class TaskEntry {

        @PrimaryKey(autoGenerate = true)     //set this attribute to be used as the primary key and have it autogenerated
        private int id;
        private String description;         //columns of the table
        private int priority;
        @ColumnInfo(name="updated_at")      //change the name of this column in the table
        private Date updatedAt;

        // Room will use this constructor since it will have an ID
        public TaskEntry(int id, String description, int priority) {
            // ...
        }

        @Ignore // We need room to ignore this constructor since we want it to use the one with the id attribute
        public TaskEntry(String description, int priority) {
            // ...
        }
    }

3. Create an abstract class to act as the Database:

    @Database(entities = {TaskEntry.class}, version = 1, exportSchema = false)  //Init the DB, give a list of classes that will be the tables
    @TypeConverters(DateConverter.class)  //Date obj cannot be stored in the DB so a TypeConverter is necessary (see step 5)
    public abstract class AppDatabase extends RoomDatabase {
        ...
        // This is a singleton that will return an instance of this database
            ...
            if (sInstance == null) {
                synchronized (LOCK) {
                    ...
                    sInstance = Room.databaseBuilder(context.getApplicationContext(),
                            ...
                            .allowMainThreadQueries()  //REMOVE THIS from production, querying in main thread will cause issues, use only for DEBUGGING
                            .build();
                }
            }
        ...
        public abstract TaskDao taskDao();  //object that handles queries (see step 4)

    }
4. Create a DAO interface to handle queries to your database:
    @Dao
...
5. (Optional) Create a Type converter to handle Date -> timestamp for the DB
    //The database cannot store a Date Object so we use this type converter to convert from a long to a Date object:
...
6. Create an instance of your database singleton class:
AppDatabase mAppDatabase = AppDatabase.getInstance(this);  //AppDatabase is the class we created in step 3

7. Use the taskDao object within your mAppDatabase object to query the database:
    // Insert a new TaskEntry object
    mAppDatabase.taskDao().insertTask(new TaskEntry(...));


-------------------------------------- Tutorials -------------------------------
#selecting_an_image
1. Create the intent:
    private static int SELECT_VIDEO = 1; //an integer to identify this activity when it returns the result

    Intent intent = new Intent(Intent.ACTION_PICK, MediaStore.Video.Media.EXTERNAL_CONTENT_URI);
    startActivityForResult(intent,SELECT_VIDEO);

2. Create the onActivityResult listener:
    // after an image is selected and we return to the main activity this event will be called and contain the selected image
    @Override
   protected void onActivityResult(int requestCode, int resultCode, @Nullable Intent data) {
      if(requestCode == SELECT_VIDEO) { //confirms the result is from video selection
          if(resultCode == RESULT_OK) {  //note that RESULT_OK is an already defined constant in AppCompatActivity
              String videoPath = data.getData().getPath();
              File videoFile = new File(videoPath);  //the video file
          }
      }
   }
--------------------------------------------------------------------------------
 #play_video
 1. select a video as seen above in #selecting_an_image
 2. Create a VideoViewer view in your layout
 3. Init the videoViewer variable and setVideoURI(data.getData()); //data.getData() as seen above
 4. Play the video with videoViewer.start();
 5. Profit

 NOTE GOALS:    SETUP Authentication so we can store data with the app in firebase storage
                Look into pyrbase the python library to access firebase storage?
